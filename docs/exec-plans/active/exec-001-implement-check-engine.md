...
Planned Task Spec: `task-0002-implement-check-command-surface`
...
Planned Task Spec: `task-0003-implement-check-index-validations`
...
Planned Task Spec: `task-0004-implement-check-boundaries`
...
This Exec Plan is complete when all three Task Specs are completed and the
repository satisfies all of the following:
...
After the final task completes:

- move this Exec Plan from `active` to `completed` when all tasks are done
- move `design-004` from `candidate` to `implemented` only after code and tests
  are fully consistent with the design
