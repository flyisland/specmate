...
This section highlights active design artefacts because they are the current
design contracts, live deltas, and roadmap anchors.

It must list at least:

- `draft` Design Docs and Design Patches
- `candidate` Design Docs and Design Patches
- `implemented` Design Docs and Design Patches
...
- which design patches are currently active
- whether a design already has associated execution work

Design artefacts in inactive end states such as `obsolete` and
`obsolete:merged` are not expanded by default in this section. They remain
visible through totals and the optional `--all` view.
...
Design Overview
  ...
  candidate
    ...
    design-008  Status Command   candidate  docs/design-docs/candidate/design-008-status-command.md
      prd: none  exec-plans: 0  direct-task-specs: 0  task-specs: 0
    design-008-patch-01-clarify-overview  Clarify Overview  candidate  docs/design-docs/candidate/design-008-patch-01-clarify-overview.md
      prd: none  exec-plans: 0  direct-task-specs: 0  task-specs: 0
  implemented
